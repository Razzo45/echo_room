        ...
        {events.length === 0 ? (
          ...
          <div className="surface-card-grid gap-5">
            {events.map((event) => (
              <div
                key={event.id}
                className="rounded-3xl border border-org-border bg-org-surface hover:bg-[#252438] hover:shadow-glowOrg transition-all p-6 h-full flex flex-col"
              >
                <Link href={`/organiser/events/${event.id}`} className="block active:scale-[0.99]">
                ...
                <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-org-border">
                  <Link
                    href={`/organiser/events/${event.id}/forum`}
                    className="text-xs text-org-accent hover:underline"
                    onClick={(e) => e.stopPropagation()}
                  >
                    Forum
                  </Link>
                  <Link
                    href={`/organiser/insights?eventId=${event.id}`}
                    className="text-xs text-org-accent hover:underline"
                    onClick={(e) => e.stopPropagation()}
                  >
                    Funnel
                  ...
                  <Link
                    ...
                    className="text-xs text-violet-100/60 hover:underline"
                    onClick={(e) => e.stopPropagation()}
                  >
                    Campaign
                  </Link>
