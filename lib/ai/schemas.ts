...
// Decision (story beat) schema (1–5 per quest). context allows "" so truncated JSON can validate.
const DecisionSchema = z.object({
  decisionNumber: z.union([z.literal(1), z.literal(2), z.literal(3), z.literal(4), z.literal(5)]),
  title: z.string().min(1),
  context: z.string().default(''),
  options: z.array(OptionSchema).min(1).max(3),
});

// Quest schema (1–5 story beats). description required for full content; truncated may have partial.
