...
export async function verifyOrganiserCredentials(
  email: string,
  password: string
): Promise<{ success: boolean; organiser?: Organiser }> {
...
async function getOrganiserSession(): Promise<string | null> {
  const cookieStore = await cookies();
  return cookieStore.get(ORGANISER_SESSION_COOKIE)?.value || null;
}

export async function getCurrentOrganiser(): Promise<Organiser | null> {
...
export async function requireAdminAuth(): Promise<Organiser> {
  const organiser = await getCurrentOrganiser();
  if (!organiser) {
...
export async function requireSuperAdminAuth(): Promise<Organiser> {
  const organiser = await getCurrentOrganiser();
  if (!organiser) {
